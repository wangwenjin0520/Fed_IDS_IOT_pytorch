...
def md5_string_encrypt(message):
    m = hashlib.md5()
    m.update(message)
    return m.hexdigest()
...
class encryption:
    ...
    def encrypt(self, filepath):
        ...
        fp.close()
        fp_tmp.close()
        md5_encrypt = self.md5_file_encrypt(filepath + '.txt')
    ...
    def md5_file_encrypt(self, filepath):
        m = hashlib.md5()
    ...
    def md5_file_verify(self, filepath, veri_code):
        m = hashlib.md5()
        file = open(filepath, 'rb')
        m.update(file.read())
        file.close()
